    ...
    try:
        ...
        with open(get_abs_path('combined_file'), 'rb') as source_file:
            for file in file_list:
                ...
                print('Building {} by splitting off {} bytes of the combined zip and saving to {}'.format(file['name'], file['size'], output_file_path))                
                # Make sure the directory we are writing to exists
                mkdir_p(os.path.join(os.getcwd(), file['name']))
                with open(os.path.join(os.getcwd(), file['name']), 'wb') as output:
